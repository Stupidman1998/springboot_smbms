...
$(function(){
	...
	var errorFile = $("#errorinfo").val();
	if(errorFile == null || errorFile == ""){
		$("#idPic_").next().text(" * 上传文件大小为：5M,格式为：jpg、jpeg、png、pneg");
	}else{
        $("#idPic_").next().text(errorFile);
	}
    var errorinfo_work = $("#errorinfo_work").val();
    if(errorinfo_work == null || errorinfo_work == ""){
        $("#workPic_").next().text(" * 上传文件大小为：5M,格式为：jpg、jpeg、png、pneg");
    }else{
        $("#workPic_").next().text(errorinfo_work);
    }
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/userCodeExist/userCode="+userCode.val(),//请求的url
